		...
		*/
		function load(path,relation,init){
			...
			dir.forEach(function(file) {
				if(params.fs.lstatSync('./'+path+file).isDirectory()){
					if(file === process.env.NODE_ENV){
						load(path+file+'/',relation,init);
					}
				}else {
					var moduleName = file.substr(0, file.indexOf('.'));
					if(moduleName !== 'rest_template'){
